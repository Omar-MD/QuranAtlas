...
- **Secondary action:** Search verses
...
**Resolved in source since inspection (require device regression pass, Phase 4):** the Furatiyyah Single→Scroll blank-reader defect (scroll container 0 px wide) and the Quran.ws ~300 px scroll column are no longer plausible in current code — the scroll strip is width 100% with cells at `min(100%, calc(100dvh*ratio))` centred, and the viewer guards zero width and retains loaded pages (`index.css:2374-2421`, `MushafPageViewer.tsx:147-150, 187-190`; fixes `ad81145`, `5afd1e9`, `52eb03a`). Do not generalise the old zero-width defect to current builds; verify on devices before closing.
...
| Severity | Finding | Concrete fix |
|---|---|---|
| Important | Searching “mercy” opened an Answer preview containing no answer or verses, only Evidence only, Mixed and evidence metadata. The preview lane now supports verse evidence cards, “Show all matches,” and Open-in-Reader actions (`SearchAnswerPreview.tsx:124-149, 193-220`); the live “mercy” result content needs a runtime re-test before this stays Critical. | Show matching verses immediately. Put a short translation/source label above the results. Keep methodology and evidence inventory under Sources. |
| Important | The empty search screen exposes Overview, Verses, Explore, Sources and disabled Save search before any query (verified: `SearchWorkspace.tsx`, `SearchHeader.tsx:57`, `SearchOverview.tsx:9-12`). | Lead with the input and a few meaningful examples. Keep the initial hierarchy simple; defer result-specific controls until they are useful. |
| Important | Results stretch across the desktop, have no obvious visible match emphasis in the sample, and use an arrow-only action for opening the reader (action verified: `SearchResultCard.tsx:23-28`; width/emphasis are runtime claims). | Constrain result width, emphasise the matched translation term, show the surah name and reference, and label the action Read verse. Preserve the query on return. |
| Critical | Reference systems differ without sufficient explanation: Furatiyyah page 3 begins at verse 2:5 in the app's mapping (verified: `data/catalog/mushaf-editions/qalun-furatiyyah-2023-v1/page-start-review.json`) while the printed page marker says 6. Search result mapping aliases Hafs references to Qalūn equivalents (`src/search/result-mapping.ts:34-77`) with no user-facing explanation. | State each numbering convention, expose equivalent references, and verify mapping for reading, copying, bookmarks and search. A reference difference is not itself proof of incorrect Qur’anic text. |
...
| Important | Both Mushaf editions occupy roughly the top 40–60% of the viewport despite Fit width being on. | Verify the aspect ratio first: blank space can be correct when fitting a complete page. Provide Full page, Fit width and Text focus with zoom as appropriate. Use available space without stretching the page or silently cropping notes. A tall phone cannot always be filled in both axes while preserving the entire page. |
...
| Critical | Reading-position retention | The same ayah/page remains in view after opening/closing drawers, returning from search, switching modes, resizing browser chrome, rotating and reopening the app. Preserve a content anchor, not just a pixel offset. |
...
| Important | Drawer fit and keyboard behaviour | Titles and Close remain accessible. Only the active drawer scrolls. Search input and results remain reachable above the keyboard; closing it restores the reading position. |
...
- **“Search is hidden inside navigation” as stated.** The drawer now exposes labelled Read/Search segmented destinations (`NavDrawer.tsx:320-333`). The remaining finding — search unavailable from reader chrome directly — is reworded in Pass 1.
...
| 3 | Critical | Expose numbering conventions and equivalent references; validate reader handoffs for search, bookmarks and copying. | High for finding and citing passages | Medium |
| 4 | Important | Show verses immediately in Search; simplify the empty state; label reader actions. | High for finding passages | Medium |
| 5 | Important | Unify Settings presentation and visual tokens; fix theme/night overlap, control labels and drawer behaviour. | High across the interface | Medium |

Reference validation applies to bookmarking and copying as well as the search work in ranks 3–4. The repaired Mushaf Scroll layouts move to Phase 4 as a device regression gate: verify at 360/390/430 px in both editions before closing. Mobile verification remains a completion gate for all five changes.
...
3. **Navigation model** (M): expose Read and Search from reader chrome on desktop; make the surah title a selector; decide drawer-overlay versus reflowing sidebar; give Bookmarks its own row. *Findings: search in navigation, drawer behaviour.*
...
6. **Search architecture** (M): simplify the empty state to input plus examples; defer Overview/Verses/Explore/Sources tabs until results exist; re-test the “mercy” answer preview and keep methodology under Sources. *Findings: empty search screen, answer preview.*
7. **Numbering conventions** (M): state each reference system, expose equivalent Hafs/Qalūn references in the reader and search, and document the mapping in About → Sources. *Findings: reference systems, mapping explanation.*
...
Rendered-surface redesign, implementing the Phase 2 structures. All theme/control changes go through the design-token layer and the UI design loop; verify contrast before adopting final colours.
...
9. **Search results presentation** (M): constrained result width; match-term emphasis; surah name and reference; labelled Read verse action; query preservation on return; labelled passage groups for multi-verse translations. *Findings: search results, passage grouping.*
...
1. **Mushaf scroll regression** (Critical): both editions, Single ↔ Scroll, at 360/390/430 CSS px and desktop; no blank pages, zero-width frames, or mode-change jumps; confirms the source-level fixes hold on devices. *Resolved-findings note.*
...
3. **Reference-mapping validation** (Critical): verify Hafs/Qalūn equivalence display and mapping correctness across reading, search, bookmarks and copying; confirm the Furatiyyah page-start mapping against printed markers.
4. **Runtime re-tests of inspection claims** (Important): “mercy” answer-preview content; search result width/emphasis; Auto-theme row wrap; header hide endpoint (−95 px peek claim); Bookmarks label at 360 px with enlarged text.
5. **Accessibility pass** (Important): contrast verification for new tokens; 44 px targets; focus-visible rings; renamed controls announced correctly; reduced-motion removes non-essential movement.
