...
```bash
git add scripts/docs docs/context/surfaces docs/context
...
```bash
while IFS= read -r path; do [ -e "$path" ] || printf 'missing %s\n' "$path"; done < <(rg -o "`[^`]+`" docs/context/style-map.md | tr -d '`' | rg '^(src|tests|docs)/')
```
...
- [ ] **Step 5: Commit skill alignment**

Run:

```bash
git add .agents/skills/quranatlas-workflow/SKILL.md .agents/skills/quranatlas-ui-workflow/SKILL.md
