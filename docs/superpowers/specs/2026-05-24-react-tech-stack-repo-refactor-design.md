...
- Offline strategy: Workbox / VitePWA with explicit app-shell precache and
  same-origin runtime assets for the single current MVP reader profile.
  User-triggered asset-pack install/activate UI is future multiple-profile work.
...
- Settings: theme, typography, translation visibility, Mushaf view mode, and
  reader comfort controls.
- Cold launch through the MVP default asset contract reset; the first-run
  source-choice onboarding wizard is retired.
- Offline-first handling for the shipped default reader assets.
- One current reader profile: Qalun in product prose; runtime ids and existing
  paths use `qaloon`. The profile is Qaloon text/font, Qaloon Mushaf, and
  Bridges translation.
- Read-only Asset Management inventory for the three default asset groups:
  Qaloon Text + Font, Qaloon Mushaf, and Bridges Translation.
- Search and navigation over the shipped reader corpus, with broader retrieval
  lanes deferred.
...
- The current MVP reader asset profile is the source of truth for
  `settings.riwayah`, `settings.quranTextStyleId`,
  `settings.mushafEditionId`, and `settings.translationId`.
- First launch under the MVP asset contract silently clears older unsupported
  local QuranAtlas settings, bookmarks, and caches once, records the contract
  marker, and opens/restores the reader.
- Hafs, Warsh, optional translations, tafsir, alternate Mushaf editions,
  source pickers, and install/verify/activate/remove controls are future
  multiple-profile work, not current React parity requirements.
- Translation data can remain Hafs-keyed internally. Current React reader parity
  renders the Qaloon default profile with Bridges translation and uses alias
  helpers where the shipped data contract requires them.
...
- reader page background, margins, body text, muted text, selection, and
  controls;
- bookmark, future curated metadata, reader selection, and reader status
  namespaces;
- offline warning, storage danger, default asset availability, and cache
  state;
...
| URL-addressed default reader dataset, text, Mushaf page, source, and search assets | Cache Storage / runtime cache from same-origin `/dataset/**` |
...
- Current shipped default reader assets must be manifest-driven, versioned,
  provenance-rich, byte-planned, and validated. User-triggered
  install-before-activate pack workflows are future multiple-profile work.
...
- The current MVP default profile is planned from generated indexes such as
  `indexes/text-assets.json`, `indexes/mushaf-assets.json`,
  `indexes/riwayah-packages.json`, source indexes where retained, and manifest
  inventory. Missing default Qaloon text, Qaloon Mushaf, or Bridges assets are
  build/runtime errors, not source-picker fallbacks.
- No silent fallback: missing, stale, unavailable, or offline-only default
  assets must produce explicit UI states or fail the relevant build/runtime
  check.
...
Pack status vocabulary remains a future-ready implementation vocabulary, but
current MVP UI exposes only the default active/read-only inventory plus explicit
missing/unavailable default-asset states. It must not expose install, verify,
activate, switch, or remove controls.

Continuity parity must preserve MVP asset-contract launch reset, valid
...
- launch with MVP default asset reset already applied;
- launch when the MVP default asset reset must run;
- retired onboarding hash redirecting through launch without source choices;
- asset management populated with the three read-only default asset groups;
- default asset missing/unavailable gate;
- reader clean mode;
...
both Verse and Mushaf settings modes, legacy onboarding hash handling, Daily
Wird detail/continue flows, search unavailable/results states, and night
recitation mode `off`, `on`, and `auto` on reader/settings/drawer proof.
...
9. **08 Offline Storage And Asset Pack Architecture**
   ...
   strategy, MVP default asset contract reset, manifest contracts, quota
   behavior, offline UI state contracts, existing IDB v7 compatibility, and
   data/source-pack verification gates. Keep user-triggered
   install-before-activate asset lifecycle as future multiple-profile
   architecture unless product scope is amended.
...
10. **08A Mushaf Install-On-Demand Asset Strategy**
    ...
    bodies in the React app artifact, edition-aware default Qaloon paths only,
    no legacy page support in React routes, executable asset-size/path checks,
    and split app/default-asset shipping. User-triggered Mushaf installs remain
    future multiple-profile work.
...
11. **09 Reader Surface Parity**
    ...
    needed. Preserve hash routes, default Qaloon/Bridges rendering, reader
    typography, and no-silent-fallback default asset states.

12. **10 Navigation, Settings, And Onboarding Parity**
    Rebuild Surah/Juz/bookmark navigation, settings, storage controls,
    and launch/onboarding compatibility against current MVP product scope.
    Settings must not reintroduce source pickers; Asset Management remains a
    read-only default inventory.

13. **11 Search And Index Parity**
    Build search over the shipped current MVP corpus: Qaloon Arabic text,
    Bridges translation, and approved navigation/index data. Broader lanes such
    as tafsir, curated metadata, transliteration, and install-before-activate
    search packs are future work unless product scope is amended. Preserve exact
    search artifact URL/cache contracts, offline unavailable states, translation
    alias resolution where required by the shipped data, and golden proof.

14. **12 Curated Metadata Parity**
    Preserve only curated reader-attached metadata lanes that remain in current
    MVP scope. Tafsir and richer curated context are future work; missing
    optional metadata must not block reader boot.

15. **13 Continuity And Bookmarks Parity**
    Preserve MVP asset-contract launch restore, valid `lastSurface`, saved
...
18. **16 Cutover Readiness**
    ...
    removing Svelte. Future multiple-profile assets, tafsir, and broader
    retrieval lanes remain out of current parity unless this master spec and
    affected child specs are amended in the same change.
