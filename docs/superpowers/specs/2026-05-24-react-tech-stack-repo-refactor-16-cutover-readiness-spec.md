...
- Depends on child specs:
  ...
  - all implementation child specs `01` through `14`
...
- Confirm every child spec acceptance criterion is met or explicitly deferred by
  product docs.
...
- `docs/product-info.md`, `docs/context/implemented.md`, `docs/tech-stack.md`,
  context docs, and agent instructions only to prepare or record readiness state.
...
- v1 product scope is implemented or product docs explicitly defer it;
...
- Pass/fail readiness checklist with evidence links for every child spec and gate.
- Concrete rollback and staging/dev soak plan.
- CI/deploy artifact-routing plan for the production flip.
...
- All deferrals are reflected in product docs and parity gates.
