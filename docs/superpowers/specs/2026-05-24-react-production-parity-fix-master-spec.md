...
- React onboarding must present only Riwayah and Translation choices, then complete setup. It must not carry the later Svelte screens for theme, shortcuts, reading preferences, offline expectations, Daily Wird entry, or similar later-screen content.
...
UI parity fixtures must include token-resolved styling proof for light, sepia, dark, and Svelte-equivalent night mode `off`, `on`, and `auto` where the surface can render under those states. Required surfaces are reader verse, Mushaf, drawer, settings, asset rows, onboarding, and Daily Wird where present. Assertions should locate elements by roles/names, then inspect computed styles or CSS variables for semantic-token resolution of canvas, surface, text, muted text, border, focus, accent, selection, danger, and reader-specific states.
...
- Active text, translation, Mushaf, metadata, and promoted search assets use install/verify-before-activate. Deleting an active required asset is blocked or atomically switches to a verified replacement before cache mutation.
...
| --- | --- | --- |
| `RPA-001` | Launch/onboarding | Clean React production-target launch routes to onboarding when `settings.onboardingComplete` is missing; onboarded launch restores valid Svelte-equivalent reader surfaces. |
...
| `RPA-005` | Settings/assets | React settings and asset management persist through the shared settings/offline contracts, restore routes like Svelte, and derive rows from real indexes/cache state. |
...
| `RPA-007` | Onboarding | React onboarding gates launch and writes setup state, but only carries Riwayah and Translation choices per the intentional exclusion above. |
...
2. Plan 01 restores launch and React-shortened onboarding. It unblocks clean-context workflow proof for every later surface.
...
- React clean launch, onboarding, navigation, settings/assets, Daily Wird, About, clear-data, and offline behavior are proven against Svelte or explicit non-carry-over rules.
