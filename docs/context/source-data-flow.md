...
The quran.ws import downloads PDFs into `.scratch/` and converts them with Poppler into the edition-scoped ignored normalized input `data/normalized/mushaf-pages/qaloon/qalun-quran-ws-v1/pages/`. A complete safe legacy ignored SVG directory is staged into that edition path without mutating the legacy directory. During the page build, the reviewed themed SVG is parsed once to derive a stable ink-framing rectangle. The builder validates that rectangle against the source `viewBox` and emits both required `viewBox` and `displayViewBox` fields for every V1 manifest page; runtime loading consumes the emitted framing instead of rescanning SVG paths.

The Furatiyyah source PDF has no network importer and remains local-only. Its import command is:
...
The private importer requires the committed passed media gate and complete runtime evidence before it checks the pinned PDF digest, document page count, and explicit `CropBox` reported by `pdfinfo -box`. It records the exact output of `pdftocairo -v`, `cwebp -version`, and `webpinfo -version`, renders only source PDF pages 5 through 608 with `pdftocairo -cropbox -png -r 300`, applies the reviewed normalized Full frame through `cwebp -crop`, emits 1,280 and 2,136 pixel WebP renditions, validates every pair with `webpinfo`, then atomically promotes a complete immutable ignored sibling at `data/normalized/mushaf-pages/qaloon/qalun-furatiyyah-2023-v1/`. Its current-only contract-v2 `import.json` binds the source-PDF digest, complete source/review/framing plus byte-affecting media-policy digest, exact tool-version provenance, and metadata content digest; runtime gate and measurement evidence remain outside byte-affecting emission identity. The importer and builder reject legacy normalized metadata rather than accepting an arbitrary legacy digest, and the builder independently verifies the current bindings and each reviewed page mapping before it can emit private output. It refuses to overwrite a different output under the same edition id.

The normalized edition is distributed as one immutable USTAR GitHub Release asset containing only its top-level edition directory, `import.json`, and 1,208 reviewed WebPs. `distribution.json` pins the exact repository, tag, asset name, archive byte count and SHA-256, normalized content and contract digests, and 1,209-file contract. The source PDF remains excluded. The source license stays restricted; a separate catalog record captures the user's narrow authorization for public noncommercial deployment of these derived WebPs in the QuranAtlas `dev`/private profile without claiming a general source license.

`pnpm run data -- mushaf-pages restore-release --archive=/absolute/path` is the local-only release restore boundary. It reads 512-byte USTAR headers itself and rejects traversal, absolute paths, links, devices, duplicate paths, unexpected roots/files, and incomplete rendition inventories before invoking `tar` against a verified staging copy. It then verifies normalized content/contract digests, exact rendition bytes, and `webpinfo` dimensions before atomically promoting the edition. Exact existing output is reusable; changed output under the immutable edition id is preserved and rejected. `restore-release --check` performs the complete descriptor and normalized/WebP validation without requiring or downloading the archive, so cache hits pass the same release boundary before use.

`mushaf-pages build` selects editions explicitly: `baseline` and `full` emit only `qalun-quran-ws-v1`; `private` emits that default plus `qalun-furatiyyah-2023-v1`; `catalog` emits none. Both private selections are inherently required, and every selected input is resolved and validated before any output, index, build-stamp, prune, or dataset-manifest write. All selected siblings build before a single riwayah-level prune, so a private build keeps both editions while a later baseline build removes the unselected private output and its index entry. Only quran.ws writes the legacy riwayah-level V1 manifest and SVG page paths, including the build-derived `displayViewBox` beside the source `viewBox`. The private edition writes a V2 manifest with canonical `firstVerse`, transformed unit `textFrame`/`sideLane`, an external-image fallback at 2,136 pixels, and both verified WebP renditions. The asset index repeats absolute `/dataset/**` URLs and each WebP descriptor's digest, byte count, dimensions, and MIME type. `--require-edition=<id>` strictly requires any other named selected edition.

`mushaf-pages build --profile=private --check` is read-only. It derives the same complete artifact model as write mode, then rejects missing, changed, or extra edition directories and page files, stale legacy default aliases, a mismatched shared asset index, or stale Mushaf membership in the dataset manifest.

CI runs the baseline Mushaf import/page-build lane when `scripts/ci/affected.mjs` detects changes to Mushaf catalogs (including private edition contracts), normalized page inputs, riwayah source files, Mushaf page scripts, the default reader asset profile, generated Mushaf runtime assets, or dependency files. CI and the shared local preview runner also run it whenever Playwright is selected so the tested baseline `dist/` artifact includes the real quran.ws SVG pages that the browser specs assert. Trusted `dev` pushes instead always restore or exactly cache and validate the ignored Furatiyyah normalized input, ensure the quran.ws input, prebuild `--profile=private`, carry that profile through affected non-Mushaf generation in `ci:build`, then run the authoritative private check against final public output before upload and private E2E against the resulting artifact. Pull requests and `staging`/`main` pushes remain baseline.
