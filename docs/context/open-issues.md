...
- **Audio shells violate token-only CSS discipline** — `src/audio/AudioFullOverlay.svelte` and `src/audio/AudioMiniBar.svelte` carry inline `<style>` blocks (violates `check-no-svelte-style.mjs`) and reference 10 undefined tokens with hardcoded fallbacks (`--qa-surface`, `--qa-text`, `--qa-border`, `--qa-surface-elev`, `--qa-surface-hover`, `--qa-bottom-nav-h`). Move CSS to `src/styles/surfaces/audio.css`; map tokens to existing semantic tokens (e.g., `--qa-surface-app`, `--qa-text-default`) or declare missing ones in `src/styles/tokens/semantic.css`. Currently blocks `pnpm validate` at the `check:styles` step.
