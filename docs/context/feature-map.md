...
- **Purpose:** Full-screen sheet on mobile + tablet (<1180px); centered modal on desktop (≥1180px). Sticky live-preview band at top updates on every change (font size, reading flow, theme palette, riwayah glyphs). Three sections in daily-use-frequency order: **Reading** (font size + reading flow sliders, translation toggle, reset link), **Appearance** (4 theme swatches, night-mode toggle), **Recitation** (collapsed-by-default Riwayah row → tap to expand 3 swatches inline). Single view — no subview navigation. Clear-data row lives on About page footer (since 2026-04-25). Translation picker UI hidden until a 2nd shipped pack lands.
- **Key behaviors:**
  - Theme swatches call `setTheme(opt)` from `settings/theme.ts`, which writes `settings.theme` and swaps `data-theme` on `<html>`.
  - **Sliders inline (no subview):** Font size (`settings/font-size.ts`) and Reading flow (`settings/reading-typography.ts::setReadingFlow` — coordinated knob that writes all four spacing keys at once). Each slider is 5-step (xs/sm/md/lg/xl). Reader picks up changes via CSS attribute selectors on `<html data-line-spacing|data-word-spacing|data-reader-margin|data-verse-spacing>` plus the existing `--qa-font-size-base` var. Margins drives `.qa-verse` horizontal padding (`--qa-verse-pad-x`); verse spacing drives vertical padding (`--qa-verse-pad-y`); on mobile `#main-content` drops its 1rem horizontal padding when `.qa-verse` is present so the slider owns the inset budget. **Reset to default** restores font size + all four flow keys to `md` and is shown only when at least one differs. The four flow IDB keys remain individually-addressable so a future advanced view could split them.
  - **Live preview band** (`.qa-settings-preview`, `data-testid="settings-preview"`) is sticky at the top of the sheet body; renders Sūrat ar-Raḥmān 1–2 in the active riwayah's glyphs (corpus-exact strings from `PREVIEW_AR` table in `Panel.svelte`). Translation line under the Arabic is gated on `settings.translationVisible`. Preview band uses a fixed warm-bronze dark background (`var(--c-bronze-900)`) regardless of active theme so it stays a constant reference frame.
  - **Recitation collapsed by default:** the Riwayah row shows the current label + sub (e.g. "Qālūn ʿan Nāfiʿ · 6214 ayāt") with `aria-expanded="false"`. Tap the row → 3 swatches mount inline (`role="radiogroup"`); chevron rotates 90°. Closing the sheet resets `recitationOpen` to `false`, so each open starts in the canonical collapsed state (set-and-forget honoured).
...
- **Layout:** mobile + tablet (<1180px) — `.qa-sheet--settings-fs` overrides the shared `.qa-sheet` shape: `inset: 0`, no border, no border-radius, safe-area insets top + bottom, `display: grid` rows = `auto auto 1fr` (header · preview · scroll body). Desktop (≥1180px) — same component, narrower modal (~440px) with `max-height: min(720px, 86vh)`; preview + body remain stacked.
- **Night mode (post 2026-04-25):** Toggle row in the Appearance section drives `data-night-mode` on `<html>`; the persistent `.qa-night-shift` overlay (mounted in `App.svelte`, styled in `styles/surfaces/night-shift.css`, `mix-blend-mode: multiply`) raises its opacity in response. Composes over any theme. Also toggleable via the `n` global reader shortcut (`nav/CommandSheet.svelte`). Sole writer of `settings.nightMode`: `settings/night-mode.ts`.
- **Recitation → Riwayah picker:** three-swatch picker (Ḥafṣ / Warsh / Qālūn) revealed on demand. Tap a swatch → `settings/riwayah.ts::setRiwayah` writes `settings['riwayah']`, emits `SETTINGS_RIWAYAH_CHANGED`, broadcasts cross-tab. Reader re-renders with the new text source, font, and line-height floor; preview band immediately reflects the new glyphs.
