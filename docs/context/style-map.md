...
React preview product components use React semantic tokens and Tailwind
utilities from `src-react/design-system/**`; they do not use Svelte CSS
partials. The active visual reference for the current Wave 09-14 pass is the
accepted current Svelte UI state named by the matching surface row above.
...
React intentionally excludes Svelte UI, such as Tafsir UI, settings preview
panes, or shortened onboarding, the active reference must be an explicit
