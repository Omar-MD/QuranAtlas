...
- **Hybrid CSS** — theme tokens and top-level shell rules stay in `src/core/theme.css` (data-theme variables, ambient chrome, sheets). Surface-specific styles are co-located in each `<style>` block inside `.svelte` files and get Svelte-scoped class hashes. Class grammar is still `qa-<surface>-<part>` (e.g. `qa-review-card-chip`, `qa-sheet-backdrop`). Themes swap CSS variables on `<html data-theme="…">`.
  - **Token-only discipline.** Component `<style>` blocks must reference tokens for every design decision: colors, radii, shadows, transitions, focus ring, backdrop blur, z-layers, typography. No hardcoded hex, no literal `border-radius: 12px`, no raw `0.2s ease`. New literal values require adding a token to `theme.css` first. Positional / one-off geometry (compound `border-radius` shapes, surface-specific paddings, unique animation keyframes) stays inline in the component.
  - **Token groups in `:root`:** spacing (`--qa-space-*`), radii (`--qa-radius-hairline/xs/sm/md/lg/xl/2xl/3xl/4xl/5xl/pill/circle`), shadows (`--qa-shadow-sm/md/lg/sheet/popover` + per-theme `--qa-ambient-elevation`), transitions (`--qa-transition-fast/base/slow`), z-layers (`--qa-z-raised/chrome/overlay/sheet-backdrop/sheet/toast/skip-link`), focus ring (`--qa-focus-ring-width/offset/color`), touch target (`--qa-touch-min`), backdrop blur (`--qa-backdrop-blur-sm/md`), typography (sizes/line-heights/font families), ambient palette (`--qa-ambient-*`), tag palette (`--qa-tag-0..11`), tag-layer hues (`--lh-*`).
