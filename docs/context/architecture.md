...
- **Service worker for offline** — `src/infra/service-worker/sw.js` + Workbox; the Quran corpus is cached in `CACHE_DATASET` and surahs load from cache first. The SW and `src/infra/offline/` helpers stay vanilla JS by design.
...
- **Dataset path and active corpus** — the reader corpus lives at `public/dataset/riwayat/{name}/{NNN}.json` (baseline: Qaloon; optional/full: Hafs and Warsh too). `src/data/dataset.ts::getSurah(n)` reads `settings['riwayah']` (sole writer `settings/riwayah.ts`, default `'qaloon'`) to resolve the URL and falls back to Qaloon when the requested non-default pack is absent from the current baseline artifact, including dev-server HTML fallbacks for missing files. Per-surah files are emitted at build time by `scripts/data/build-dataset.mjs` from normalized sources under `data/normalized/**`; the script regenerates `surahs.json`, `juz.json`, `indexes/sources.json`, `manifest.json`, and `provenance.json`. The separate knowledge lane builder (`scripts/data/build-knowledge-dataset.mjs`) emits `public/dataset/knowledge/**` shards consumed lazily by `src/data/knowledge-dataset.ts`, and those shards now flow through the verified dataset manifest so the existing offline/update pipeline can cache them under the Text category without changing reader boot. Run via `pnpm run data -- build`; `pnpm run build` chains that data build into the app bundle.
