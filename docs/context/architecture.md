...
- **Dataset path and active corpus** — the reader corpus lives at `public/dataset/riwayat/{name}/{NNN}.json` (baseline: Qaloon; optional/full: Hafs and Warsh too). `src/data/dataset.ts::getSurah(n)` reads `settings['riwayah']` (sole writer `settings/riwayah.ts`, default `'qaloon'`) to resolve the URL and falls back to Qaloon when the requested non-default pack is absent from the current baseline artifact, including dev-server HTML fallbacks for missing files. `scripts/data/cli.mjs` orchestrates the lane builders: `scripts/data/text/build.mjs` regenerates `surahs.json`, `juz.json`, `indexes/sources.json`, `manifest.json`, and `provenance.json`, while `scripts/data/knowledge/build.mjs` emits `public/dataset/knowledge/**` shards consumed lazily by `src/data/knowledge-dataset.ts`. `manifest.json` is now an inventory with lane/category/byte metadata; offline planning uses that inventory for membership and byte totals instead of digests. Run via `pnpm run data -- build`; `pnpm run build` chains that data build into the app bundle.
- **Translation pipeline** — translation packs ship at `public/dataset/translations/{id}/{NNN}.json`. Source files live at `data/normalized/translations/{id}.json` (outside `public/`, build-only input, never shipped) and are produced by the generic catalog-driven fetcher (`pnpm run data:fetch -- translation:saheeh`) and committed so the build stays offline. `dataset.ts::loadTranslationForSurah(id, surahNo)` returns the per-surah pack (verses + footnote map + intro paragraphs); `Reader.svelte` joins it onto rendered verses by `verseKey` and passes the surah-wide footnote map into each `Verse.svelte`. The current committed Saheeh pack ships without footnote entries, but the runtime still supports inline `[N]` markers when a future shipped pack includes them. Schema + invariants live in `data-model.md` §Translation packs.
