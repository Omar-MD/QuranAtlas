...
**Mobile / tablet variant (<1180px) — single-row redesign 2026-04-25, drawer overhauled 2026-04-25, label retuned 2026-04-26, sizes bumped 2026-04-27:** Ambient dock is hidden entirely. Primary navigation is `MarginHeader` at the top, ~56 px tall — left **hamburger ≡** (48px tap target, 26px icon) opens the nav drawer, center **single-line Arabic surah label** in `'Amiri Quran'` Mushaf script (18 px; the previous uppercase smallcaps English subline was dropped 2026-04-26), right **settings gear ⚙** (48px tap target, 26px icon). No second-row tabs, no kebab, no fast-tag dot. Header auto-hides on scroll down and reveals on scroll up or any `AMBIENT_SURFACE` emit. `#main-content` reserves ~60 px of top padding. **Gestures:** **tap label = toggle in-reader Surah Header visibility** (post 2026-04-26 — was no-op; sole writer `settings/surah-header-visibility.ts`, IDB key `surahHeaderHidden`, preference persists across surahs; bismillah + verses keep rendering); swipe left/right on label = next/prev surah (clamped 1–114); swipe down on header = `openNavDrawer('read')` (no longer hash navigation); single-tap gear = settings sheet (debounced 300 ms so a follow-up tap can suppress); double-tap gear (≤300 ms between taps) = cycle theme (parity with keyboard `d`). Replaced 2026-04-26 long-press; `MarginHeader` no longer arms a long-press timer. **Regression guard:** `tests/e2e/journey-b-reader.spec.js` 'mobile margin header is a single row, ≤ 60 px tall' + `tests/unit/nav/MarginHeader-toggle.test.ts` (label tap → toggle, off-route no-op, wordmark-mode no-op, keyboard Enter parity, swipe-still-navigates regression guard, English line dropped).

**Mobile drawer (full-screen, restructured 2026-04-28):** Hamburger or swipe-down opens a full-screen drawer with two top-level mode tabs — **Read** (default) · **Study**. Read mode has two sub-tabs: **Surahs** (default) · **Bookmarks**. Surahs sub-tab: search input + filter pills (All / ⏱ Recent) + scrolling surah list, auto-scrolled to and highlighting the currently-reading surah. Each row shows the number badge, English name, and **Arabic surah title** (`s.name_ar`, RTL, `--qa-font-arabic`) right-aligned. Bookmarks sub-tab: verse-level rows grouped by surah (canonical order, ascending verse within section); each row shows the `surah:verse` ref + a truncated Arabic snippet (RTL). Tap a row → drawer closes, reader navigates to that verse, landing pulses 1s. Swipe-left a row → reveals Delete button (mobile); desktop hover-`×`. Empty-state copy: "Tap a verse number in the reader to bookmark it." Bookmarks scope to the active riwayah — switching riwayah surfaces a different set. Study mode: top **Hub** row (→ `#/review`) + 12 layer rows in 4 grouped sections (Speech / Narrative / Themes / Entities) — tapping a layer navigates to `#/review?layer=<name>`. Header carries a tappable QuranAtlas wordmark + ⓘ icon → `#/about`. ✕ closes; backdrop tap, swipe-left, Esc also dismiss. Drawer state is local; not persisted. **Regression guards:** `tests/unit/nav/drawer.test.ts` (Study tab structure, search-text filter, ref-jump hint + Enter, verseNum filter, Arabic-title-on-every-row, Bookmarks empty-state, Bookmarks grouped-rows after seed) + `tests/unit/bookmarks/store.test.ts` (add/del/toggle, riwayah scoping, grouped-by-surah) + `tests/e2e/journey-f-navigation.spec.js` `F-mobile-1` (current-surah highlight + Read+Surahs default) / `F-mobile-2` (Study layer-tap navigates) / `F-mobile-3` (wordmark→About) / `F-mobile-4` (deep-link redirect) / `F-mobile-5` (center-label no-op).
...
1. **Mobile + tablet (<1180px):** single-tap the gear `⚙` on the right side of `MarginHeader` → Settings sheet **takes over the full viewport** (`inset: 0`, safe-area insets on all four edges, no border). Portrait stacks preview → body → footer; **landscape (max-height ≤540px and orientation: landscape)** switches to a side-by-side grid — preview occupies a left rail (full height with theme-true bg), body + footer stack on the right column with their own scroll. Double-tap gear (≤300 ms apart) cycles theme without opening the sheet.
...
3. Sheet content (three zones, no scroll under default content):
   - **Live preview band** at the top — theme-true colors (preview = `var(--qa-surface-raised)`, Arabic = `var(--qa-text-arabic)`). Sūrat ar-Raḥmān 1–4 rendered in the active riwayah's glyphs. The Arabic uses the `.qa-verse-arabic` class and the translation uses `.qa-verse-translation`, so font-size + reading-flow sliders drive the preview through the same cascade the reader uses (font-size, line-height, word-spacing all match exactly). The translation row is always rendered but its visibility is gated on `settings.translationVisible` — the row keeps its layout space when toggled off, so the preview height does not shift on toggle. The ✕ close button floats inside the band's top-right.
   - **Body** — two sections, each `flex: 1` (balanced). Soft hairline gold-fade separator between them.
     - **Reading** — Font size slider (5-step), Reading flow slider (5-step coordinated knob). Reset-to-default pill button lives in the section header's right edge, **always rendered** (disabled = idle when both knobs are at `md`); flipping in/out of default never reflows the slider rows. Preview band itself is hard-locked at 42dvh portrait / 38dvh landscape / 240px desktop modal — the inner stage scrolls when verse content overflows so band height never changes.
...
4. The sticky preview band at the top of the sheet shows Sūrat ar-Raḥmān 1–4 in the active Riwayah — characters + tashkeel encoding match what the reader will produce. The Arabic font is whatever the active Riwayah is bound to (Hafs → KFGQPC Hafs, Warsh → KFGQPC Warsh, Qaloon → KFGQPC Qaloon — see the Reader Arabic-font invariant in section B). Both Arabic and translation share the reader's typography classes (`.qa-verse-arabic` + `.qa-verse-translation`), so every reading-flow knob (font size, line spacing, word spacing) drives the preview in lockstep with the reader.
