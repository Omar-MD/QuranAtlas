...
> **Invariant (Riwayah + reader text source).** Reader text source = active Riwayah (from `settings['riwayah']`, default Qālūn). Sole writer of `settings['riwayah']` is `settings/riwayah.ts`. Font follows via `--qa-font-arabic` cascade (set by `:root[data-riwayah=...]` overrides — all three resolve to Amiri Quran). Reader's reading-typography slider drives line-height; floor at the `xs` step clears stacked harakat across all riwayat. Each `.qa-verse-arabic` element carries `data-riwayah` mirroring the active Riwayah for CSS specificity.

> **Invariant (Reader Arabic font default, post 2026-04-27).** Default Arabic font = **Amiri Quran** (Khaled Hosny, OFL) for all three riwayat — single Mashriqi Naskh face cross-engine, full Quranic mark coverage (fatha, kasra, damma, sukun, shadda stacks, dagger alif `U+0670`, small high seen `U+06EC`, alif waslah `ٱ`, sajdah-cue marks `U+06D6+`), professionally hinted, identical pixel-grid render on Chromium (Skia), WebKit (CoreGraphics — macOS Safari, iOS Safari, iOS Chrome, headless WebKit), and Firefox (Gecko). Wired through `--ff-amiri-quran` token (`src/styles/tokens/primitives.css`) → `--qa-font-arabic` (`src/styles/tokens/semantic.css`).
>
> **Per-Riwayah opt-in alternates (post 2026-04-28).** Each Riwayah can override the default via the typography settings picker (D5) to either `KFGQPC <Riwayah>` (Hafs v22 / Warsh V21 / Qaloon V21 — purpose-built mushaf cuts) or `Scheherazade New` (SIL academic). Selection persists per-Riwayah via `arabicFont_{riwayah}` IDB keys; `<html data-arabic-font>` reflects the active Riwayah's choice and is reapplied on `SETTINGS_RIWAYAH_CHANGED`. The KFGQPC trio was retired 2026-04-27 due to an iOS hollow-mark rendering bug and re-introduced 2026-04-28 as opt-in only — iPhone users opting in accept the rendering risk; default remains Amiri Quran. See `docs/context/riwayat-dataset.md` § "Cross-engine rendering" for the historical context. **Regression guards:** `tests/unit/settings/arabic-font.test.ts` (per-Riwayah load/apply/set + Riwayah-change reapplication) and `tests/unit/settings/panel.test.ts` D5 picker behaviour.
...
### D5. Typography subview (font size · Arabic font · reading flow)
...
2. Subview top: live preview block (one Arabic verse + translation line) wrapped in the reader's column rules so margin and spacing changes are visible inside the sheet. The Arabic preview text is the active Riwayah's actual rendering of Sūrat ar-Raḥmān 1–2 — same characters and tashkeel encoding the reader will produce, so the preview is a true sample (not a generic Naskh string).
3. **Font size** slider — 5-step (xs / sm / md / lg / xl), writes `fontSize` IDB key, drives `--qa-font-size-base`.
4. **Arabic font** picker (post 2026-04-28) — three swatches: `Amiri Quran` (default), `KFGQPC <Riwayah>` (the active Riwayah's purpose-built KFGQPC mushaf cut — Hafs v22 / Warsh V21 / Qaloon V21), and `Scheherazade` (SIL academic). Selection persists per-Riwayah via `arabicFont_hafs` / `arabicFont_warsh` / `arabicFont_qaloon` IDB keys; only the active Riwayah's choice is reflected on `<html data-arabic-font>` at any one time. Switching Riwayah from the main Settings view (or from anywhere via `setRiwayah`) reapplies that Riwayah's stored font choice via the `SETTINGS_RIWAYAH_CHANGED` listener and re-renders the picker labels (`KFGQPC Ḥafṣ` → `KFGQPC Warsh` etc.). KFGQPC trio was retired 2026-04-27 for an iOS hollow-mark bug then re-introduced 2026-04-28 as opt-in; iPhone users selecting KFGQPC accept that risk. CSS chain: `--qa-font-arabic` resolves via `[data-riwayah][data-arabic-font]` selectors in `tokens/semantic.css`.
5. **Reading flow** slider — 5-step coordinated knob: a single drag writes all four spacing keys (`lineSpacing`, `wordSpacing`, `readerMargin`, `verseSpacing`) to the same step via `setReadingFlow(step)`. CSS attribute selectors on `<html data-…>` re-render the reader live. The four IDB keys remain individually-addressable so a future advanced view can split them again. Arabic line-height = `1.92 + delta(step)` — the floor (xs) clears KFGQPC tashkeel collisions across all riwayat, md (default) lands at 2.12.
6. **Reset to default** appears below the sliders only when at least one knob (font size or reading flow) ≠ `md`. Tap → font size + all four flow keys return to `md` and the button hides. Arabic font picker is **not** part of the reset (per-Riwayah preference is preserved across resets).
7. Back arrow returns to main Settings view; subtitle reflects new state.

**Surfaces:** Settings sheet (Typography subview), Reader. **Persistence:** `settings.fontSize`, `settings.lineSpacing`, `settings.wordSpacing`, `settings.readerMargin`, `settings.verseSpacing`, `settings.arabicFontHafs`, `settings.arabicFontWarsh`, `settings.arabicFontQaloon`. **Sole writers:** `src/settings/reading-typography.ts` (line/word/margin/verse-spacing); `src/settings/arabic-font.ts` (per-Riwayah font). **Regression guards:** `tests/unit/settings/{panel,reading-typography,arabic-font}.test.ts` (subview structure, slider → IDB, reset button, persist round-trip, picker labels re-render on Riwayah change) + `tests/e2e/journey-d-settings.spec.js` 'D5: reading-flow …' (real-CSS line-height / word-spacing / max-width assertions).
