...
Cross-surah continuation links (`↑ <prev>` and `<next> ↓`) sit nearly flush against the scroller's top/bottom edges at `opacity: 0.4`; reveal to full opacity + accent on hover/focus. Pull-to-swap is the primary touch affordance (see B-Cross).
