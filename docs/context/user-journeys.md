...
> **Invariant (Reader Arabic font, post 2026-04-27).** All three riwayat render in **Amiri Quran** (Khaled Hosny, OFL) — single Mashriqi Naskh face cross-engine, full Quranic mark coverage (fatha, kasra, damma, sukun, shadda stacks, dagger alif `U+0670`, small high seen `U+06EC`, alif waslah `ٱ`, sajdah-cue marks `U+06D6+`), professionally hinted, identical pixel-grid render on Chromium (Skia), WebKit (CoreGraphics — macOS Safari, iOS Safari, iOS Chrome, headless WebKit), and Firefox (Gecko). No engine-conditional fallback, no `data-engine` attribute, no asset post-processing. Wired through `--ff-amiri-quran` token (`src/styles/tokens/primitives.css`) → `--qa-font-arabic` (`src/styles/tokens/semantic.css`). KFGQPC fonts retired same date — see `docs/context/riwayat-dataset.md` § "Cross-engine rendering" for the history. **Regression guard:** `tests/unit/styles/font-tokens.test.js` asserts the riwayah → Amiri Quran wiring (validated via Rule 5 break-and-restore).
