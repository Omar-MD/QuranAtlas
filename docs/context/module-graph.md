...
- **Imports from:**
  ...
  - `core/aliases.ts` → `src/data/aliases.json`
...
- **Role:** Main reading surface (Svelte 5), split into focused modules:
  ...
  - `global-position.ts` — **sole writer** for `settings.currentPosition` (data-model.md cross-cutting rules). Single global record (current surah + verse), supersedes the legacy per-surah `positions` store dropped in DB v4.
...
- **Role:** Svelte component routes. Both the review hub (12-layer filter) and FVR live in `Hub.svelte` — branches on whether the `layer` + `value` props are present. `ReviewCard.svelte` is the per-mark card (async verse content loaded on mount); chip links use `#/threads/<tag>`. `state.ts` is the **sole writer** for `meta["review"]` (data-model.md cross-cutting rules) — persists view/activeLayer/activeValue/filter/sort/groupBy.
