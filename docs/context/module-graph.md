...
- **Role:** Cross-cutting primitives. `db.ts` (IDB + strict `StoreRecords` types + `LayerName` / `LAYER_NAMES` / `MarkRecord`), `events.ts` (pub/sub + typed `EventPayloads`), `router.ts` (hash routing), `constants.ts` (Events enum + payload typedefs + shared constants), `logger.ts` (noop wrapper in tests, console in prod), `normalize.ts` (canonicalization pipeline — `normalize()` + `canonicalize()`), `aliases.ts` (alias map + `excludeFromAliasing` guard), `ui.svelte` (undo toast) + `ui-bridge.ts` (imperative `showUndoToast()`), `tag-colors.ts` (deterministic tag-color mapping), `quota-banner.svelte` (storage warnings). All CSS now lives under `src/styles/` (entry `styles/index.css`) — tokens + shell rules split across `styles/tokens/*.css` + `styles/base.css`; surface-specific CSS migrates from component `<style>` blocks into `styles/surfaces/*.css` per PRs 2–13 — `core/quota-banner.svelte` → `styles/surfaces/quota-banner.css` (PR 2 landed); shared sheet + modal chrome → `styles/surfaces/sheet.css` + `styles/surfaces/modal.css` with context-scoped token inheritance roots (PR 3 landed); app-shell layout + dock rail → `styles/surfaces/app-shell.css` (PR 4 landed — only file allowed `:has()`); `about/About.svelte` → `styles/surfaces/about.css` (PR 5 landed; about-heading `:has()` rule lives in app-shell.css); `onboarding/Onboarding.svelte` → `styles/surfaces/onboarding.css` (PR 6 landed — theme-swatch hex retained as demo literals); `surahs/SurahList.svelte` + `surahs/SurahRow.svelte` → `styles/surfaces/surahs.css` (PR 7 landed); `nav/{AmbientDock,AmbientPill,CommandSheet,MarginHeader,SurahProgress,TagModePill}.svelte` → `styles/surfaces/nav.css` (PR 8 landed — check-token-usage script now reads `.svelte` `style:--X=` directives + `--X:` inline attribute declarations into the global token set); `reader/{Reader,Verse,VerseTags}.svelte` + imperative `edge-indicators.ts` selectors → `styles/surfaces/reader.css` (PR 9 landed); `tag/{TagSheet,AmbientDock,TagModeToggle,TagChip,VerseSpotlight}.svelte` → `styles/surfaces/tag.css` (PR 10 landed — `color-mix()` surface policy exercised here; TagChip dot hue now piped via `style:--qa-tag-chip-hue` directive instead of `!important` escape hatch); `marks/{Editor,TagChip,TagLayerRegion}.svelte` → `styles/surfaces/marks.css` (PR 11 landed — `:global(.qa-sheet--mark)` overrides promoted to plain descendant selectors against the global sheet carrier); `review/{Hub,ReviewCard}.svelte` → `styles/surfaces/review.css` (PR 12 landed — filter-bar chip `button` nested selectors required `selector-max-compound-selectors: 3` per-file override); `settings/Panel.svelte` → `styles/surfaces/settings.css` + legacy `.qa-error-state` / `.qa-retry-btn` / `.qa-undo-toast` → `styles/surfaces/toast.css` (PR 13 landed — CSS refactor complete: `_legacy.css` deleted, alias shims retired, `scripts/check-no-svelte-style.mjs` gate forbids `<style>` in any `.svelte` file, `pre-css-refactor-baseline` branch removed).
