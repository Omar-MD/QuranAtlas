...
- Runtime guards in `src/data/dataset.ts`: saved riwayah ids absent from the baseline fall back to `qaloon`; translation/tafsir ids fall back to `bridges` / `muyassar` only when their same-origin pack fetch fails. `getTranslations()` and `getTafsirs()` read from `indexes/sources.json`, not `provenance.json`, so Settings can expose optional packs even when the current manifest does not include their bodies.
