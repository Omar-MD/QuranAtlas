...
Mushaf page assets are edition-aware. Quran.ws manifest V1 requires both the source SVG `viewBox` and a build-derived `displayViewBox` for every inline-safe page body. The display rectangle is finite, positive, contained by the source rectangle, and supplies stable Reader framing without runtime path scanning. The private Furatiyyah edition uses manifest V2: each of 604 pages declares its canonical `firstVerse`, a unit Full-relative `textFrame` and side lane, plus `external-image` media with a 2,136-wide fallback and both SHA-256-described WebP renditions. The asset index repeats those descriptor values with absolute same-origin URLs. `src/packs/mushaf-index.ts`, `src/packs/mushaf-paths.ts`, and `src/packs/mushaf-page-asset.ts` validate:
...
`loadPreparedMushafPage` is the V1/V2 preparation boundary. It selects inline SVG or external-image preparation from the validated manifest version rather than from exception text. One active-profile context fully validates and shares the asset index, edition manifest, page count, framing capability, and edition identity across the five-page logical window. Each entry then performs only cheap profile-identity and selected-page descriptor/index checks before fetching its page body or rendition; the complete 604-page/1,208-rendition agreement is not rescanned for every prepared page. The requested page and immediate neighbors prepare concurrently, while the two outer pages stay metadata-only until promoted. V1 outer entries perform no SVG body work. V2 current and neighboring entries prepare the 1,280-pixel rendition first; the current page then promotes to 2,136 pixels while retaining readable preview media through upgrade failure. The V2 boundary resolves only manifest-declared edition-relative WebP paths, derives a page's final verse from `verseToPage`, and prepares an image only after browser load and `decode()` complete. The viewer mounts V1 as sanitized inline SVG using `displayViewBox` and V2 as an external image. `mushafPageFraming` is a bounded 0–1 settings value; framing controls are available only when the shared active V2 profile validates reviewed framing on every page.
