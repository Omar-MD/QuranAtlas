...
- Dataset profiles:
  - `baseline`: emits Qaloon riwayah, Bridges translation, Muyassar tafsir, core metadata, source index, manifest, provenance.
...
- Runtime guards in `src/data/dataset.ts`: saved riwayah/translation/tafsir ids that are absent from the baseline fall back to `qaloon`, `bridges`, and `muyassar` respectively instead of failing the reader. `getTranslations()` and `getTafsirs()` read from `indexes/sources.json`, not `provenance.json`, so Settings can expose optional packs even when the current manifest does not ship their bodies.
