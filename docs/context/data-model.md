...
`loadPreparedMushafPage` is the V1/V2 preparation boundary. It selects inline SVG or external-image preparation from the validated manifest version rather than from exception text. The reader loads one validated asset-index/manifest context per active profile and shares it across its five-page logical window, so entries fetch only their page body or rendition rather than refetching profile metadata. The requested page is current, adjacent pages are decode-gated previews, and the two outer pages are validated descriptors until promoted. The V2 boundary resolves only manifest-declared edition-relative WebP paths, derives a page's final verse from `verseToPage`, and prepares an image only after browser load and `decode()` complete. The viewer mounts V1 as sanitized inline SVG and V2 as an external image. `mushafPageFraming` is a bounded 0–1 settings value; framing controls are available only when the active V2 manifest validates reviewed framing on every page.
