...
- **Verse mode** (`#/s/:surah/:ayah?`) mounts `Reader.svelte`, sets `reader.readerMode = 'verse'`, clears `reader.currentMushafPage`, and owns verse scroll/translation/tafsir behavior. The read surface must render the active verified qira'ah/riwayah + Quran text-style bundle only. Qalun is the baseline product pack; runtime keys continue to use `qaloon`. Verse text resolves through `/dataset/quran-text/{riwayah}/{quranTextStyleId}/{surah}.json` after `src/packs/text-assets.ts` confirms the selected text asset exists and is usable. Hafs and Warsh are optional. Missing or stale optional text/page/search assets show unavailable/install/switch states or explicitly switch the active setting to a verified baseline before rendering baseline content. During the React dual-build parity track, `src-react/app/routes/read/ReaderRoute.tsx` is a route container around `ReaderVerseSurface`. It reads the existing React settings mirror for `riwayah`, `quranTextStyleId`, `translationId`, `translationVisible`, `readerMargin`, and `verseSpacing`, then loads Quran text from the same variant-aware dataset path. React reader proof uses the root `public/dataset/**` tree served at same-origin `/dataset/**` by `vite.react.config.js` during dev and preview; dataset bodies are not copied into `dist-react/`. React reader rendering intentionally omits Tafsir preview and full-sheet UI while preserving base Quran text, Bridges translation, footnote disclosure, Basmala rules, optional knowledge metadata, and explicit unavailable/error states when required text is missing. React mirrors the Svelte reader column caps, centered Surah header, Basmala container sizing, verse padding variables, and `/dataset/surahs.json` quick navigation chevrons for default and settings-driven reader margin / verse spacing states. React `ReaderChrome` is a single row with hamburger, flexible spacer, compact Verse/Mushaf mode icon, and settings; it intentionally has no center Surah/Page title or title click action, and the Surah header/description remains rendered in the reader content. The Verse chrome auto-hides on downward scroll and reveals on upward scroll. Switching Verse→Mushaf resolves the explicit route ayah, the live centered verse from document scroll, or the current persisted verse through the active Mushaf manifest `verseToPage`; switching Mushaf→Verse resolves the active page's `firstVerse`. The current React reader uses normal document scroll for the single-surah verse list while `useReaderPositionSync` mirrors Svelte's reader progress contract by tracking the centered visible verse, persisting `settings.currentPosition`, and letting mode switches preserve the user's reading location. Browser proof asserts no nested virtual scroller is mounted, preventing scroll lock or jump while parity work continues.
