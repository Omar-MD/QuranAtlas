...
`MushafRoute` resolves the active `riwayah` and `mushafEditionId`, validates one shared index/manifest profile context, sanitizes V1 SVGs, and maintains a retained window of at most five entries around the requested page. The requested page is current, its immediate neighbors are decode-gated previews, and outer entries remain descriptors until promoted; when a V2 preview becomes current, the visible page is upgraded to the full rendition. A failed or unavailable destination keeps the ready page mounted and authoritative for the reader label, page counter, bookmark, and Reading view bridge. A viewport-owned, safe-area-aware polite status names the failed destination and offers reachable `Retry page N` and `Stay on page M` actions without covering permanent reader controls or introducing document scroll; staying restores the visible page hash. The asset gate appears only when no visible page exists, and its Manage assets action opens the owned Settings asset inventory only when that behavior is supplied. V2 pages mount as external images with reviewed Full/Text framing; the page itself remains a labeled image without decorative framing.
...
Single + Fit page is contained by the Mushaf shell's dynamic viewport: the shell fills `100dvh`, clips document overflow, and leaves the stage without scroll-region semantics, focusability, or vertical wheel, touch, and keyboard movement. Horizontal flicks still turn exactly one ready page. Fit width and Scroll keep the document fixed while exposing `Scrollable Mushaf pages` as the focusable native vertical stage.
