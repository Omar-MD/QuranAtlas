...
| `src/app/routes/read/mushaf-page-window-state.ts` | _(no leading comment)_ |
| `src/app/routes/read/useMushafPageWindow.ts` | _(no leading comment)_ |
| `src/app/routes/read/useMushafProfileSession.ts` | _(no leading comment)_ |
...
| `src/components/reader/useMushafChromeVisibility.ts` | _(no leading comment)_ |
...
`MushafRoute` resolves the active `riwayah` and `mushafEditionId`, validates one shared index/manifest profile context once, retains cheap identity and selected-page media checks on every preparation, sanitizes V1 SVGs, and maintains a retained window of at most five entries around the requested page. The requested page and immediate neighbors prepare concurrently; the two outer entries remain metadata-only until promoted. V1 outer entries do not fetch, parse, scan, or serialize SVG bodies. V2 current and neighboring pages commit their 1,280-pixel readable rendition first, and a current page then promotes in place to 2,136 pixels without discarding its preview if the upgrade fails.

A discrete request for an unready destination keeps the current readable page mounted, queues one replaceable destination, and commits automatically through the normal route-history path as soon as that page is readable. A failed or unavailable destination keeps the ready page authoritative for the reader label, page counter, bookmark, and Reading view bridge. Network, `5xx`, and decode failures retry after 150 and 500 milliseconds; confirmed missing and contract failures do not retry automatically. Loading, retrying, exhausted transient failure, contract failure, and confirmed missing retain distinct truthful outcomes. A viewport-owned, safe-area-aware polite status names a terminal destination and offers reachable `Retry page N` and `Stay on page M` actions without introducing document scroll; staying restores the visible page hash. The asset gate appears only when no visible page exists, and its Manage assets action opens the owned Settings asset inventory only when that behavior is supplied. V2 pages mount as external images with reviewed Full/Text framing; the page itself remains a labeled image without decorative framing.

| Navigation | Fit page | Fit width |
| --- | --- | --- |
| Single | Fits the complete page inside the full viewport. | Fills the safe inline width and gives the stage native vertical scrolling until the complete page is reachable. |
...
The app wrapper, Mushaf shell, and Mushaf surface share one clipped `100dvh` viewport. The page stage uses all of it; top and bottom chrome are safe-area-aware overlays and never reserve rows, padding, or calculated page height. Single + Fit page has no scroll-region semantics or sequential-focus stop and ignores vertical wheel, touch, and keyboard movement, while remaining programmatically focusable for safe handoff when chrome hides. Horizontal flicks still turn exactly one ready page. Fit width and Scroll keep the document fixed while exposing `Scrollable Mushaf pages` as the focusable native vertical stage. Non-Mushaf routes retain ordinary document-flow minimum height.

Single mode recognizes horizontal intent only after real pointer movement distinguishes it from a native vertical pan. Finger-right, the left page action, and `ArrowLeft` advance to the higher page; finger-left, the right page action, and `ArrowRight` return to the lower page. Distance or deliberate recent velocity completes a turn after the correct neighbor previews and settles; short, cancelled, interrupted, unavailable-neighbor, and outward-boundary gestures settle back without changing the route. A center-stage tap toggles the shared overlay chrome, while scrolling alone never reveals it. A committed Single-page navigation resets the incoming stage to the top; resize, orientation, and Fit-width changes clamp the same page's existing offset instead.
...
`ReaderInteractionContext` suspends Mushaf pointer and keyboard handling while Settings or Navigation is open and cancels an interaction already in progress. The safe top overlay keeps Navigation at the physical left, the retained visible page's Arabic Surah title at geometric viewport center, and Daily Wird, Reading view, and Settings at the physical right; narrow portrait moves the title to its own centered row. The safe bottom dock groups the higher-page action, page counter, lower-page action, and page bookmark. Phone portrait uses 48-pixel targets, short landscape uses 44–48-pixel targets, and wide screens keep one centered page without side controls.

Mushaf page bookmarks use the same bookmarks store as verse bookmarks with `kind: 'page'` and a synthetic `verseKey` of `m:<page>`.

### Reader Chrome

`ReaderChrome` is compact and mode-aware: navigation drawer, Settings, Daily Wird status when enabled, and exactly one icon-only `ReadingViewToggle` on actual Verse or Mushaf reader routes. Its accessible action names the destination view; Settings, Search, About, and other non-reader routes do not render it. In Mushaf mode the top overlay and bottom dock share one session-scoped visibility controller. They appear when the first readable page commits, hide after one 2,500-millisecond discovery interval, and do not rearm for page turns, rendition promotion, retries, resize, rotation, or preference changes. Page commits hide them immediately; center tap toggles them and Escape reveals them without starting a timer. Focus, Navigation, Settings, and terminal recovery pause or pin visibility. Hidden chrome is inert, pointer-inactive, absent from sequential focus and the accessibility tree, and the bottom dock is unmounted. Reduced motion removes overlay and page travel without changing visibility timing. The compact Daily Wird status reports today's assigned progress only; full-plan progress and remaining completion gap stay in the navigation drawer.
