...
Window-side package installation is owned by `src/data/offline.ts`. It plans package bytes from the package index, pre-flights `navigator.storage.estimate()`, writes text URLs to `CACHE_DATASET`, writes page manifest/SVG URLs to the route-derived edition-aware page cache, emits package-specific progress/errors, and verifies cache membership before `settings.riwayah` can change. Failed fetch or cache writes leave the active riwayah and previous-usable install intent untouched. Removing optional Hafs/Warsh deletes both cache groups; Qalun (`qaloon`) removal is refused, and active optional removal first verifies and switches to Qalun (`qaloon`).
...
- **Optional package removal preserves a usable active riwayah.** Qalun (`qaloon`) cannot be removed. If Hafs or Warsh is active, removal must first switch to verified Qalun (`qaloon`); otherwise the delete is refused.
