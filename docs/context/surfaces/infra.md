...
- **`settings.offlineCategories` is the source of truth for "user opted into category X".** Text opt-in is source-aware (`text.riwayat`, `text.translations`, `text.tafsir`) with a migration from the former `{ hafs, warsh, qaloon }` text shape. `getActivationState()` reports `'cached'` when any category is opted in; `'downloading'` while a `CACHE_DATASET` is in flight.
