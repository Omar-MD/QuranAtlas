...
- **Active optional concrete asset deletion is refused.** `removeTextAsset` and `removeMushafAsset` throw `Switch to another compatible asset before deleting.` before touching Cache Storage when the target is the active optional text style or Mushaf edition.
