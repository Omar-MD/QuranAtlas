...
14. ✅ **Per-asset-class SW routing + per-feature offline opt-in selector** (R-11, C-4, N21) — landed 2026-05-01; spec at `docs/superpowers/specs/2026-05-01-n21-sw-aggregator-offline-selector-design.md`. `src/core/sw/route-defs.ts` carries the declarative `ROUTE_DEFS` table + `CACHE_PREFIXES` registry + `categoryFor` / `cacheNameFor` / `sumBytesForCategory` helpers (window-importable, no workbox); `src/core/sw/strategies.ts::registerAll()` bridges the table to workbox at SW boot. `src/sw.js` now delegates routing entirely. `src/offline/offline-selector.svelte` mounts in the new Settings Storage section; per-category opt-in via accordion (`<details>`) with manifest-byte estimates and pre-flight quota refusal. `src/data/offline.ts::startCategoryDownload(cat)` + `initOfflineMigration()` wipe-and-re-opt-in. Manifest gains `fileSizes` map (additive — pre-existing readers untouched). Regression guards: `tests/unit/sw/route-defs.test.ts` (table precedence + byte-sum), `tests/unit/settings/offline-categories.test.ts` (sole writer), `tests/unit/offline/offline-selector.test.ts` (5 component tests), `tests/e2e/journey-h-offline.spec.js` H2 (@offline real shell + reload re-hydrate).
...
17. ✅ **CSP migration to HTTP headers + `connect-src` per-feature allow-list framework** (R-19, R-32) — header migration shipped 2026-04-29 (`public/_headers`); allow-list registry landed 2026-05-01 at `docs/context/csp-allowlist.md` with per-feature attribution per directive + forbidden-token list (`'unsafe-eval'`, `'wasm-unsafe-eval'`, `'unsafe-hashes'`, wildcards, third-party origins without privacy review). Regression guard at `tests/unit/safety/csp-headers.test.ts` rejects directive drift, undeclared widening, forbidden tokens, and missing defence-in-depth headers (X-Frame-Options, COOP, CORP, Permissions-Policy). `unsafe-inline` removal stays at P3.24 (R-19c, deferred to v1.3 inline-style audit).
...
19. ✅ **Update-poll `lastPollAt` debounce** (R-26) — landed 2026-05-01. `src/core/sw-update-poll.ts::MIN_GAP_MS = 5 min` floor between polls; visibility/focus/30-min-interval all funnel through `pollUpdate()`. Regression guard at `tests/unit/core/sw-update-poll.test.ts` (back-to-back polls within 5 min collapse to one `reg.update()` call).
20. ✅ **Vocabulary dictionary** (R-17) — landed 2026-05-01. `docs/context/glossary.md` covers mark/tag, layer/facet, canonicalize/_canon, verseKey, riwayah/riwayat/reciter, surface/route/view, settings keys, bridge (post-N22 factory), sole writer, topic. Reciter vs riwayah disambiguation locked before audio.
