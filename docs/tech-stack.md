...
| `pnpm run data -- mushaf-pages restore-release --check` | Validate the complete private distribution pointer and existing normalized metadata/WebPs without downloading the archive; trusted `dev` CI runs this on cache hits and misses before use |
...
Affected-change and branch-profile decisions live in `scripts/ci/affected.mjs` so CI and local validation share the same path groups and only a trusted `push` to `dev` selects the private Mushaf lane. That lane reads the committed distribution descriptor, caches the ignored normalized edition by its complete archive SHA only, downloads the exact GitHub Release asset on a miss, restores it through the local USTAR boundary, then validates the complete descriptor and normalized/WebP output on both cache hits and misses. It ensures quran.ws normalized pages, prebuilds the private profile, carries that profile through affected non-Mushaf generation in `ci:build`, and checks final public Mushaf output before upload. It also forces production-preview E2E with the private flag. PR, `staging`, and `main` builds retain the affected baseline behavior.
