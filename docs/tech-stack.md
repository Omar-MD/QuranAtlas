...
| `pnpm run preview:react` | Serve `dist-react/` on port 4175; `vite.react.config.js` also serves the root `public/dataset/**` tree at same-origin `/dataset/**` for React parity proof without copying dataset bodies into `dist-react/`. React font faces import the shared `public/fonts/**` assets through the React design-system CSS so preview/build typography matches the Svelte app. React proof service-worker caches use the `quranatlas-react-proof-*` Workbox prefix plus `quran-atlas-react-runtime-dataset-v1` for runtime dataset responses. |
