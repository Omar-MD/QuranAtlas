...
export function validateSourceCatalog(catalog) {
  ...
  const authorityIds = new Set(authorities.map((a) => a?.id).filter(Boolean))
  const licenseById = new Map(licenses.map((l) => [l?.id, l]).filter(([id]) => Boolean(id)))
  ...
  for (const source of sources) {
    ...
    if (!isRecord(source.sourceChecksum)) {
      errors.push(`source ${source.id} missing sourceChecksum`)
    } else {
      if (source.sourceChecksum.algorithm !== 'sha256') {
        errors.push(`source ${source.id} checksum algorithm must be sha256`)
      }
      if (typeof source.sourceChecksum.value !== 'string' || !/^[a-f0-9]{64}$/.test(source.sourceChecksum.value)) {
        errors.push(`source ${source.id} checksum value must be 64 hex chars`)
      }
    }
    ...
    if (source.fetch !== undefined) {
      if (!isRecord(source.fetch)) {
        ...
        if (typeof source.fetch.pinPath !== 'string' || !source.fetch.pinPath) {
          errors.push(`source ${source.id} fetch missing pinPath`)
        }
