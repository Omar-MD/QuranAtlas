 ...
 *
 * Output: `data/catalog/translation-verse-aliases.json` (tracked canonical
 * anchor; text build copies it into the shipped dataset).
 *
 * Algorithm:
...
async function main() {
  ...
  for (const meta of surahsMeta) {
    ...
    const hafs = splits.hafs[pad3(n)]
    const warsh = splits.warsh[pad3(n)]
    const qaloon = splits.qaloon[pad3(n)]
  ...
  // Minified — saves ~50% (376 KB → ~180 KB) on first reader open. Schema
  // mirrors the shared translation types; pretty-printing buys nothing.
  await writeFile(join(CATALOG_DIR, 'translation-verse-aliases.json'), JSON.stringify(output), 'utf8')
