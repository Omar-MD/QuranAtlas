...
export function parseAyahKey(value) {
  ...
  ensure(rest.length === 0, `Invalid ayah key "${value}"`)
  const surah = Number(surahRaw)
  const ayah = Number(ayahRaw)
  ensure(Number.isInteger(surah) && Number.isInteger(ayah), `Invalid ayah key "${value}"`)
  ensure(surah >= 1 && surah <= 114 && ayah >= 1, `Invalid ayah key "${value}"`)
  return { surah, ayah }
}
...
function assertAyahExists(ayahKey, surahAyahCounts, context) {
  const { surah, ayah } = parseAyahKey(ayahKey)
  const maxAyah = surahAyahCounts[surah - 1]
  ensure(Number.isInteger(maxAyah) && maxAyah > 0, `${context}: missing ayah count metadata for surah ${surah}`)
  ensure(ayah <= maxAyah, `${context}: ayah key "${ayahKey}" exceeds surah ${surah} ayah count (${maxAyah})`)
  return { surah, ayah }
}
...
export async function writeKnowledgeArtifacts(artifacts, outputDir = KNOWLEDGE_OUTPUT_DIR) {
  ...
  for (let surah = 1; surah <= 114; surah++) {
    const key = pad3(surah)
    await Promise.all([
      writeFile(join(ayahDir, `${key}.json`), JSON.stringify(artifacts.ayahBySurah[key]), 'utf8'),
      writeFile(join(passagesDir, `${key}.json`), JSON.stringify(artifacts.passagesBySurah[key]), 'utf8'),
    ])
  }

  await Promise.all([
    writeFile(join(indexesDir, 'theme-to-ayah.json'), JSON.stringify(artifacts.indexes.themeToAyah), 'utf8'),
    writeFile(join(indexesDir, 'ayah-to-passage.json'), JSON.stringify(artifacts.indexes.ayahToPassage), 'utf8'),
    writeFile(join(indexesDir, 'passage-to-ayah.json'), JSON.stringify(artifacts.indexes.passageToAyah), 'utf8'),
  ])
}
