...
export async function loadSurahAyahCounts() {
  ...
  for (const row of hafs) {
    ...
    'Hafs source must cover all 114 surahs',
  )
  return counts
}
...
function validateThemes(themes) {
  ...
  for (const theme of themes) {
      ...
      typeof theme.label.en === 'string' && theme.label.en.trim().length > 0,
      `theme "${theme.id}" missing label.en`,
    )
    ensure(
      typeof theme.description === 'string' && theme.description.trim().length > 0,
      `theme "${theme.id}" missing description`,
    )
    if (theme.parentId !== null && theme.parentId !== undefined) {
      ensure(
        typeof theme.parentId === 'string' && theme.parentId.length > 0,
        `theme "${theme.id}" parentId must be a string or null`,
      )
    }
    if (theme.aliases !== undefined) {
      ensure(Array.isArray(theme.aliases), `theme "${theme.id}" aliases must be an array`)
      ensure(
        theme.aliases.every((alias) => typeof alias === 'string' && alias.length > 0),
        `theme "${theme.id}" aliases must be non-empty strings`,
      )
    }
    if (theme.related !== undefined) {
      ensure(Array.isArray(theme.related), `theme "${theme.id}" related must be an array`)
      ensure(
        theme.related.every((id) => typeof id === 'string' && id.length > 0),
        `theme "${theme.id}" related ids must be strings`,
      )
    }
...
function validatePassages(passages, themeById, surahAyahCounts) {
  ...
  for (const passage of passages) {
    ...
    seenIds.add(passage.id)
      ...
      Number.isInteger(passage.surah) && passage.surah >= 1 && passage.surah <= 114,
      `passage "${passage.id}" has invalid surah`,
    )
    ensure(
      typeof passage.startKey === 'string' && typeof passage.endKey === 'string',
      `passage "${passage.id}" must define startKey and endKey`,
    )
    ...
    const end = assertAyahExists(passage.endKey, surahAyahCounts, `passage "${passage.id}" endKey`)
      ...
      `passage "${passage.id}" range crosses surahs (${passage.startKey} -> ${passage.endKey})`,
    )
    ensure(
      start.surah === passage.surah,
      `passage "${passage.id}" surah ${passage.surah} does not match range surah ${start.surah}`,
    )
    ensure(start.ayah <= end.ayah, `passage "${passage.id}" startKey must be <= endKey`)

    ensure(
      isRecord(passage.title) && typeof passage.title.en === 'string' && passage.title.en.trim().length > 0,
      `passage "${passage.id}" missing title.en`,
    )
    ensure(
      isRecord(passage.summary) && typeof passage.summary.en === 'string' && passage.summary.en.trim().length > 0,
      `passage "${passage.id}" missing summary.en`,
    )
    ensure(
      typeof passage.roleInSurah === 'string' && passage.roleInSurah.trim().length > 0,
      `passage "${passage.id}" missing roleInSurah`,
    )
    ...
    for (const themeId of passage.themes) {
      ...
      REVIEW_STATUSES.has(passage.source.reviewStatus),
      `passage "${passage.id}" has invalid reviewStatus "${passage.source.reviewStatus}"`,
    )
  ...
  for (let surah = 1; surah <= 114; surah++) {
    ...
    for (let i = 1; i < ranges.length; i++) {
      ...
      const curr = ranges[i]
        ...
        curr.startAyah > prev.endAyah,
        `overlapping passages are not allowed in Phase 01 (${prev.id} overlaps ${curr.id})`,
      )
    }
  }
  ...
  for (const row of ayahThemes) {
    ...
    const normalizedThemes = row.themes.map((theme) => {
      ...
      seenThemeIds.add(theme.id)
        ...
        typeof theme.weight === 'number' && Number.isFinite(theme.weight),
        `ayah-themes "${row.ayahKey}" theme "${theme.id}" weight must be a number`,
      )
      ensure(
        theme.weight >= 0 && theme.weight <= 1,
        `ayah-themes "${row.ayahKey}" theme "${theme.id}" weight must be between 0 and 1`,
      )
      ensure(
        typeof theme.source === 'string' && AYAH_THEME_SOURCES.has(theme.source),
        `ayah-themes "${row.ayahKey}" theme "${theme.id}" source must be one of: ${[...AYAH_THEME_SOURCES].join(', ')}`,
      )
      ensure(
        typeof theme.certainty === 'string' && CERTAINTY_VALUES.has(theme.certainty),
        `ayah-themes "${row.ayahKey}" theme "${theme.id}" certainty must be one of: high, medium, low`,
      )
...
function sortObjectKeys(input, compare = (a, b) => a.localeCompare(b)) {
  return Object.fromEntries(Object.entries(input).sort(([a], [b]) => compare(a, b)))
}
...
export function buildKnowledgeArtifacts({ themes, passages, ayahThemes, surahAyahCounts }) {
    ...
    surahAyahCounts.every((count) => Number.isInteger(count) && count > 0),
    'surahAyahCounts entries must be positive integers',
  )
...
export async function main() {
  ...
  const [surahAyahCounts, sourceData] = await Promise.all([loadSurahAyahCounts(), loadKnowledgeSeedData()])
