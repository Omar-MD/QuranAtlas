...
function safeArchivePath(path) {
    ...
    `Private Mushaf USTAR contains an unsafe absolute path: ${path}`,
  )
  const parts = path.replace(/\/$/, '').split('/')
  ensure(
    parts.every((part) => part && part !== '.' && part !== '..'),
    `Private Mushaf USTAR contains unsafe traversal: ${path}`,
  )
  return parts.join('/')
}
...
function readUstarEntries(buffer) {
    ...
    buffer.byteLength >= BLOCK_BYTES * 2 && buffer.byteLength % BLOCK_BYTES === 0,
    'Private Mushaf release archive is not block-aligned USTAR',
  )
  const entries = []
  let offset = 0
  let ended = false
  while (offset < buffer.byteLength) {
    ...
    if (header.every((byte) => byte === 0)) {
        ...
        'Private Mushaf USTAR contains data after its end marker',
      )
    ...
    const magic = header.subarray(257, 263)
      ...
      magic.equals(Buffer.from('ustar\0')) || magic.equals(Buffer.from('ustar ')),
      'Private Mushaf release archive is not USTAR',
    )
    ...
    const type = typeByte === 0 ? '0' : String.fromCharCode(typeByte)
      ...
      type === '0' || type === '5',
      `Private Mushaf USTAR entry ${rawPath} has forbidden type ${JSON.stringify(type)}`,
    )
...
function validateDistribution(distribution) {
    ...
    distribution?.version === 1 && distribution.mushafEditionId === PRIVATE_EDITION_ID,
    'Private Mushaf distribution identity is invalid',
  )
  ensure(
    distribution.authorization === 'user-authorized-public-noncommercial-deployment' &&
      distribution.repository === 'Omar-MD/QuranAtlas' &&
      distribution.releaseTag === PRIVATE_RELEASE_TAG &&
      distribution.assetName === PRIVATE_ASSET_NAME,
    'Private Mushaf distribution release pointer is invalid',
  )
  ensure(
    Number.isSafeInteger(distribution.archiveBytes) && distribution.archiveBytes > 0,
    'Private Mushaf distribution archive byte count is invalid',
  )
  ensure(
    /^[a-f0-9]{64}$/.test(distribution.archiveSha256 ?? ''),
    'Private Mushaf distribution archive SHA-256 is invalid',
  )
  ensure(
    /^[a-f0-9]{64}$/.test(distribution.normalizedContentDigest ?? '') &&
      /^[a-f0-9]{64}$/.test(distribution.normalizedContractDigest ?? ''),
    'Private Mushaf distribution normalized digests are invalid',
  )
  ensure(
    Number.isSafeInteger(distribution.fileCount) && distribution.fileCount > 0,
    'Private Mushaf distribution file count is invalid',
  )
}
...
function validateNormalizedMetadata(bytes, distribution) {
  ...
  try {
    ...
    metadata?.version === 1 &&
      metadata.emissionContractVersion === 2 &&
      metadata.riwayah === 'qaloon' &&
      metadata.mushafEditionId === distribution.mushafEditionId,
    'Private Mushaf release normalized identity is invalid',
  )
  ensure(/^[a-f0-9]{64}$/.test(metadata.sourcePdfSha256 ?? ''), 'Private Mushaf release source PDF digest is invalid')
  ensure(
    ['pdftocairo', 'cwebp', 'webpinfo'].every(
      (tool) => typeof metadata.toolVersions?.[tool] === 'string' && metadata.toolVersions[tool].length > 0,
    ),
    'Private Mushaf release tool provenance is incomplete',
  )
  ensure(
    metadata.contractDigest === distribution.normalizedContractDigest,
    'Private Mushaf release normalized contract digest is invalid',
  )
  ensure(
    metadata.contentDigest === distribution.normalizedContentDigest,
    'Private Mushaf release normalized content digest is invalid',
  )
  const { contentDigest, ...unsignedMetadata } = metadata
    ...
    Array.isArray(metadata.pages) && metadata.pages.length > 0,
    'Private Mushaf release normalized pages are incomplete',
  )
  ensure(
    distribution.fileCount === 1 + metadata.pages.length * 2,
    'Private Mushaf release normalized page count disagrees with its distribution file count',
  )
  const renditions = []
  for (let index = 0; index < metadata.pages.length; index += 1) {
    ...
    const row = metadata.pages[index]
      ...
      `Private Mushaf release page ${page} renditions are incomplete`,
    )
    ensure(
      Number.isInteger(row.sourcePdfPage) &&
        Number.isInteger(row.firstVerse?.surah) &&
        Number.isInteger(row.firstVerse?.verse),
      `Private Mushaf release page ${page} source mapping is invalid`,
    )
...
export function inspectPrivateMushafTar(buffer, distribution) {
  validateDistribution(distribution)
    ...
    buffer.byteLength === distribution.archiveBytes,
    'Private Mushaf release archive byte count does not match its distribution descriptor',
  )
  ensure(
    sha256(buffer) === distribution.archiveSha256,
    'Private Mushaf release archive SHA-256 does not match its distribution descriptor',
  )
  ...
  for (const entry of entries) {
      ...
      entry.path === distribution.mushafEditionId || entry.path.startsWith(`${distribution.mushafEditionId}/`),
      `Private Mushaf USTAR contains an unexpected root: ${entry.path}`,
    )
  ...
  const pagesDir = entryByPath.get(`${distribution.mushafEditionId}/pages`)
    ...
    editionDir?.type === '5' && pagesDir?.type === '5',
    'Private Mushaf USTAR is missing its exact edition directories',
  )
  ...
  for (const rendition of renditions) {
    ...
    const entry = entryByPath.get(path)
      ...
      entry.bytes.byteLength === rendition.bytes && sha256(entry.bytes) === rendition.sha256,
      `Private Mushaf USTAR rendition ${rendition.assetPath} bytes are invalid`,
    )
  }
  const regularFiles = entries.filter((entry) => entry.type === '0')
  ensure(
    regularFiles.length === distribution.fileCount,
    'Private Mushaf USTAR file count does not match its distribution descriptor',
  )
  ensure(
    entryByPath.size === expectedPaths.size && [...entryByPath.keys()].every((path) => expectedPaths.has(path)),
    'Private Mushaf USTAR contains an unexpected file or incomplete inventory',
  )
  return {
...
async function validateExtractedEdition(editionDir, inspected, runCommand) {
  ...
  const expectedFiles = new Set(['import.json', ...inspected.renditions.map((item) => item.assetPath)])
    ...
    inventory.directories.size === 1 && inventory.directories.has('pages'),
    'Private Mushaf restored output contains unexpected directories',
  )
  ensure(
    inventory.files.size === expectedFiles.size && [...inventory.files].every((path) => expectedFiles.has(path)),
    'Private Mushaf restored output contains an unexpected file or incomplete inventory',
  )
  const importBytes = await readFile(join(editionDir, 'import.json'))
  ensure(
    importBytes.equals(inspected.importBytes),
    'Private Mushaf restored import.json differs from the verified archive',
  )
...
export async function validatePrivateMushafRelease({
  ...
  distribution,
    ...
    typeof normalizedRoot === 'string' && isAbsolute(normalizedRoot),
    'Private Mushaf release validation requires an absolute normalized root',
  )
...
export async function restorePrivateMushafReleaseArchive({
  ...
  distribution,
    ...
    'Private Mushaf release restore requires an absolute local archive path',
  )
  ensure(
    typeof normalizedRoot === 'string' && isAbsolute(normalizedRoot),
    'Private Mushaf release restore requires an absolute normalized root',
  )
  const descriptor = distribution ?? (await readDistribution())
  ...
  try {
    ...
    if (existsSync(normalizedDir)) {
      try {
        ...
        throw new Error(
          `Private Mushaf immutable output already exists with different bytes: ${error instanceof Error ? error.message : String(error)}`,
        )
      }
      return { normalizedDir, status: 'current' }
    }
    try {
      ...
      try {
        ...
        throw new Error(
          `Private Mushaf immutable output appeared with different bytes: ${validationError instanceof Error ? validationError.message : String(validationError)}`,
        )
      }
      return { normalizedDir, status: 'current' }
    }
