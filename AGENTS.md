...
Detailed rules are in `.opencode/rules/testing.md` and `.opencode/rules/definition-of-done.md` (loaded automatically when editing `src/` or `tests/`). Run `/verify` before committing to check all criteria.
...
Available skills for streamlined workflows:
...
- **`/qa`** — Interactive QA session: user reports bugs, agent files GitHub issues
- **`/tdd`** — Test-driven development with red-green-refactor loop
- **`/triage-issue`** — Investigate a bug, find root cause, create TDD fix plan as GitHub issue
...
- **`/handoff`** — Save session state to progress.md for a fresh agent
- **`/ubiquitous-language`** — Extract DDD-style domain glossary

The spec-driven development loop: load spec → implement → verify → commit → ship
...
**AGENTS.md** — When a decision during implementation changes any of the following, ask the user before the conversation ends whether AGENTS.md should be updated:
...
Do NOT silently update AGENTS.md. State what changed, quote the specific section(s) that are now stale, and propose the edit. Only apply it after explicit approval.
