  ...
  {#if summary.state === 'no-plan' || editing}
    <div class="qa-wird-setup">
      <div class="qa-wird-targets" role="group" aria-label="Completion target">
        <button type="button" data-testid="wird-target-7" class:qa-wird-option--on={targetMode === 'preset' && targetDays === 7} onclick={() => { targetMode = 'preset'; targetDays = 7 }}>7 days</button>
        <button type="button" data-testid="wird-target-30" class:qa-wird-option--on={targetMode === 'preset' && targetDays === 30} onclick={() => { targetMode = 'preset'; targetDays = 30 }}>30 days</button>
        <button type="button" data-testid="wird-target-90" class:qa-wird-option--on={targetMode === 'preset' && targetDays === 90} onclick={() => { targetMode = 'preset'; targetDays = 90 }}>90 days</button>
        <button type="button" data-testid="wird-target-custom" class:qa-wird-option--on={targetMode === 'custom'} onclick={() => { targetMode = 'custom'; targetDays = null }}>Custom date</button>
      </div>
      ...
      <div class="qa-wird-start" role="group" aria-label="Start point">
        <button type="button" aria-pressed={startMode === 'current'} onclick={() => { startMode = 'current' }}>Current position <span>{currentRefLabel}</span></button>
        <button type="button" aria-pressed={startMode === 'beginning'} onclick={() => { startMode = 'beginning' }}>Beginning <span>1:1</span></button>
      </div>
