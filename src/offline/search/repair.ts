...
export async function reconcileSearchPackState(
  ...
  if (active?.status === 'active') {
    if (await caches.has(searchPackCacheName(active.contentHash))) {
      ...
      if (compatible && compatible.contentHash !== active.contentHash)
        return { state: 'update available', activePack: compatible }
      return { state: 'active', activePack: compatible }
    }
    return online
      ? { state: 'available online', activePack: null, reason: 'active Search pack cache missing' }
      : { state: 'offline unavailable', activePack: null }
  }

  const staged = await db.searchPackStaging.toArray()
  const latestStaged = staged.sort((a, b) => b.updatedAt - a.updatedAt)[0]
  if (latestStaged)
    return { state: latestStaged.status.replace('-', ' ') as SearchPackAvailabilityState, activePack: null }
  if (!online) return { state: 'offline unavailable', activePack: null }

  const registry = await fetchSearchPackRegistry(options.fetcher)
  const compatible = selectCompatibleSearchPack(registry)
  return compatible
    ? { state: 'available online', activePack: compatible }
    : { state: 'incompatible', activePack: null }
}
