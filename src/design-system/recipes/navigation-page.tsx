...
export function NavigationPageRecipe({
  ...
  return (
    <main className="qar:grid qar:gap-3 qar:px-5 qar:py-4" aria-label={title}>
      <header className="qar:grid qar:gap-1">
        {kicker ? <p className="qar:m-0 qar:text-sm qar:text-muted">{kicker}</p> : null}
        <h1 className="qar:m-0 qar:text-xl qar:font-semibold qar:leading-tight qar:text-text">{title}</h1>
      </header>
      {children}
    </main>
  )
