...
describe('font-size', () => {
  beforeEach(async () => {
    vi.resetModules()
    await openDB()
    try { await del('settings', 'fontSize') } catch {}
  })
  ...
  it('maps legacy "small" → "sm" and "large" → "lg"', async () => {
    ...
    const { loadFontSize: lfs1 } = await import('../../../src/settings/font-size.js')
    expect(await lfs1()).toBe('sm')

    vi.resetModules()
    await put('settings', { key: 'fontSize', value: 'large' })
    const { loadFontSize: lfs2 } = await import('../../../src/settings/font-size.js')
